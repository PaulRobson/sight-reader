...
- [x] Attempt metronome: one-bar count-in then a steady click at the piece tempo, armed by the "Let's go" gesture. **Done:** `useMetronome` is a Web Audio lookahead scheduler (accented downbeat per bar, plain click between, so the first bar reads as the count-in before the steady click); `useAttemptMetronome` drives it from view state — clicks through `playNow` when `metronomeOnAttempt` is on, stops on leaving — and returns `arm()`, called inside the "Let's go" gesture to unlock iOS audio before the (non-gesture) countdown-zero start. `beatSpecFromAbc` derives the click interval + beats-per-bar from the tune's `M:`/`Q:1/4=` headers (compound metres felt in dotted-quarter beats, cut time in half beats); tested. Hook audio is covered by the Slice 5 manual checkpoint.
